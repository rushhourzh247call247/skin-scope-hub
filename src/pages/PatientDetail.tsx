...
const PatientDetail = () => {
  ...
  const handleMapClick = (x: number, y: number) => setMapClickDialog({ x, y });
      ...
      <header className="border-b bg-card">
        <div className="container flex h-16 items-center gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-base font-semibold text-foreground">{patient.name}</h1>
              <p className="flex items-center gap-2 text-xs text-muted-foreground">
                <span className="font-mono">ID: #{patient.id}</span>
                <span>•</span>
                <span className="tabular-nums">
                  Geb. {patient.birth_date ? format(new Date(patient.birth_date), "dd. MMM yyyy", { locale: de }) : "–"}
                </span>
              </p>
            </div>
          </div>
      ...
      <main className="container py-6">
        <div className="grid gap-6 lg:grid-cols-[1fr_320px_1fr]">
          ...
          <div className="space-y-4">
            <h3 className="text-sm font-medium text-foreground">Körperstellen ({locations.length})</h3>
            <div className="space-y-1.5">
              {locations.length === 0 ? (
                ...
                locations.map((loc) => (
                  ...
                  >
                    <MapPin className={`h-4 w-4 shrink-0 ${selectedLocationId === loc.id ? "text-primary" : "text-muted-foreground"}`} />
                    <div className="min-w-0 flex-1">
                      <p className="truncate text-sm font-medium text-foreground">{loc.name || `Stelle #${loc.id}`}</p>
                      <p className="flex items-center gap-2 text-xs text-muted-foreground">
                        <span className="font-mono">x:{loc.x} y:{loc.y}</span>
                        <span>•</span>
          ...
          <div className="space-y-3">
            <h3 className="text-center text-sm font-medium text-foreground">Body Map</h3>
            <div className="rounded-md border bg-card p-4">
              <BodyMapSvg
                markers={locations.map((l) => ({ id: l.id, x: l.x, y: l.y, name: l.name }))}
                onMapClick={handleMapClick}
                selectedLocationId={selectedLocationId}
              />
            </div>
            <p className="text-center text-xs text-muted-foreground">Klicken Sie auf den Körper, um eine neue Stelle zu markieren</p>
          </div>
          ...
          <div>
            <AnimatePresence mode="wait">
              {selectedLocation ? (
                <motion.div key={selectedLocation.id} initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }} transition={{ duration: 0.15 }}>
        ...
        <DialogContent>
          ...
          <div className="space-y-4 pt-2">
            ...
            <div className="space-y-2">
              ...
              <Input id="locName" placeholder="z.B. Linker Unterarm" value={locationName} onChange={(e) => setLocationName(e.target.value)} />
            </div>
