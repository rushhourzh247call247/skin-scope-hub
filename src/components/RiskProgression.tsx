...
const RiskProgression = ({ images, locationName }: RiskProgressionProps) => {
  ...
  function getRiskLabel(level: string | null | undefined): string {
    if (!level) return "–";
    if (level === "low") return t('riskProgression.low');
    if (level === "medium") return t('riskProgression.medium');
    return t('riskProgression.high');
  }
    ...
    return (
      <div className="rounded-md border bg-popover px-3 py-2 text-xs shadow-md">
        ...
        <p style={{ color: getRiskColor(data.risk_score) }}>
          {t('riskProgression.score')}: {data.risk_score ?? "–"} ({getRiskLabel(data.risk_level)})
        </p>
      </div>
    );
  };
  ...
  const trendLabel = trend === "up" ? t('riskProgression.worsening') : trend === "down" ? t('riskProgression.improvement') : t('riskProgression.stable');

  return (
    <div className="space-y-3 rounded-lg border bg-card p-3">
      <div className="flex items-center justify-between">
        <h4 className="text-xs font-semibold text-foreground uppercase tracking-wider">
          {t('riskProgression.title')}
        </h4>
        {trend && (
      ...
      {everHigh && (
        <div className="flex items-center gap-1.5 rounded-md border border-red-200 bg-red-50 px-2 py-1.5 text-[11px] font-medium text-red-700">
          ⚠️ {t('riskProgression.highRiskWarning')}
        </div>
      )}
        ...
        <div className="h-28 w-full">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={chartData} margin={{ top: 5, right: 10, left: -20, bottom: 0 }}>
              ...
              <YAxis domain={[0, 5]} ticks={[0, 1, 2, 3, 4, 5]} tick={{ fontSize: 9, fill: "hsl(var(--muted-foreground))" }} axisLine={false} tickLine={false} />
              ...
              <Line type="monotone" dataKey="risk_score" stroke="hsl(var(--foreground))" strokeWidth={2} dot={<CustomDot />} connectNulls />
            </LineChart>
          </ResponsiveContainer>
        </div>
      )}
      ...
      {hasAnyScore && chartData.length === 1 && (
        <div className="flex items-center gap-2 text-xs">
          ...
          >
            {t('riskProgression.score')}: {chartData[0].risk_score}
          </span>
      ...
      <div className="flex gap-2 overflow-x-auto pb-1">
        {sorted.map((img) => (
          <button key={img.id} onClick={() => setSelectedImage(img)} className="relative shrink-0 group">
            <div className="h-14 w-14 overflow-hidden rounded-md border-2 border-border transition-all group-hover:border-primary">
              <img src={api.resolveImageSrc(img)} alt="" className="h-full w-full object-cover" loading="lazy" />
            </div>
            {img.risk_score != null && (
              <span
        ...
        <DialogContent className="max-w-md p-2">
          {selectedImage && (
            <div className="space-y-2">
              <img src={api.resolveImageSrc(selectedImage)} alt="" className="w-full rounded-md object-contain max-h-[60vh]" />
              <div className="flex items-center justify-between px-1">
                <span className="text-xs text-muted-foreground">
                  {selectedImage.created_at ? formatDate(selectedImage.created_at, "dd.MM.yyyy HH:mm") : "–"}
                </span>
                {selectedImage.risk_score != null && (
                  ...
                  >
                    {t('riskProgression.score')}: {selectedImage.risk_score} – {getRiskLabel(selectedImage.risk_level)}
                  </span>
                )}
              </div>
            </div>
